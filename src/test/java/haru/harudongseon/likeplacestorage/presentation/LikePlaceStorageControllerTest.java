...
class LikePlaceStorageControllerTest extends E2ETest {
    ...
    class FindAllLikePlaces {
        ...
        void success() {
            ...
            // when
            final ExtractableResponse<Response> response = FIND_ALL_LIKE_PLACES_REQUEST(accessToken, likePlaceStorage.getId());
            final StoredLikePlacesResponse actual = response.as(StoredLikePlacesResponse.class);
        ...
        @Test
        @DisplayName("장소 보관함에 장소가 없는 경우 빈 리스트로 조회된다.")
        void success_empty_list() {
            // given
            final Member member = memberBuilder.defaultMember().build();
            final String accessToken = jwtService.createAccessToken(member.getId());
            ...
            assertSoftly(softly -> {
                softly.assertThat(response.statusCode()).isEqualTo(HttpStatus.OK.value());
                softly.assertThat(actual.getStoredLikePlaces()).isEmpty();
            });
        }

        @Test
        @DisplayName("멤버 ID와 장소 보관함 ID에 해당하는 장소 보관함이 존재하지 않으면 실패한다.")
        void fail_not_exist_like_place_storage() {
            // given
            final Member member = memberBuilder.defaultMember().build();
            final String accessToken = jwtService.createAccessToken(member.getId());
            ...
            // when
            final ExtractableResponse<Response> response1 = FIND_ALL_LIKE_PLACES_REQUEST(notExistMemberAccessToken, likePlaceStorage.getId());
            final ExtractableResponse<Response> response2 = FIND_ALL_LIKE_PLACES_REQUEST(notExistMemberAccessToken, notExistLikePlaceStorageId);
            final ExtractableResponse<Response> response3 = FIND_ALL_LIKE_PLACES_REQUEST(accessToken, notExistLikePlaceStorageId);

            // then
            assertSoftly(softly -> {
                softly.assertThat(response1.statusCode()).isEqualTo(HttpStatus.UNAUTHORIZED.value());
                softly.assertThat(response1.jsonPath().getString("errorMessage")).contains("인증에 실패했습니다. 정확한 에러는 서버 로그를 확인해주세요.");
                softly.assertThat(response2.statusCode()).isEqualTo(HttpStatus.UNAUTHORIZED.value());
                softly.assertThat(response2.jsonPath().getString("errorMessage")).contains("인증에 실패했습니다. 정확한 에러는 서버 로그를 확인해주세요.");
                softly.assertThat(response3.statusCode()).isEqualTo(HttpStatus.NOT_FOUND.value());
                softly.assertThat(response3.jsonPath().getString("errorMessage")).contains("장소 보관함 ID와 멤버 ID에 해당하는 장소 보관함이 존재하지 않습니다.");
            });
        }
    }
    ...
    private static ExtractableResponse<Response> FIND_ALL_LIKE_PLACES_REQUEST(final String accessToken, final Long likePlaceStorageId) {
        return RestAssured.given().log().all()
                .header(HttpHeaders.AUTHORIZATION, JWT_PREFIX + accessToken)
                .when().log().all()
