...
public record RoutePlaceDto(
        ...
        @Schema(description = "장소 이름", nullable = false, example = "스타벅스 부평점")
        String name,
        ...
        @Schema(description = "장소 주소 이름", nullable = false, example = "인천광역시 부평구 경원대로 1397")
        String addressName,

        @NotBlank(message = "전화번호는 공백일 수 없습니다. 정보가 없다면 NONE을 입력하세요.")
        @Schema(description = "장소 전화번호", nullable = false, example = "1522-3232")
        String phoneNumber,

        @NotBlank(message = "웹사이트는 공백일 수 없습니다. 정보가 없다면 NONE을 입력하세요.")
        @Schema(description = "장소 웹사이트", nullable = false, example = "http://www.starbucks.co.kr/")
        String website,

        @NotBlank(message = "장소 구글 검색 URL은 공백일 수 없습니다.")
        @Schema(description = "장소 구글 검색 URL", nullable = false, example = "https://maps.google.com/?cid=9514396914460199663")
    ...
    public Place toEntity() {
        ...
        return new Place(providerPlaceId, name, category, photoReferences, coordinates, openingHours, addressName, phoneNumber, website, googleMapsUri, placeDetails);
    }
}
